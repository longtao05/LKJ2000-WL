...
def Cal_Crc32(puiData,ulSize,ulInitValue):
    uloop = 0
    puiDataCal = puiData
    ulCrc = ulInitValue
...
def Bytes_To_List(data_bytes):
    #未考虑字节不能被4整除情况，
    data_len=math.ceil(len(data_bytes)//4)
    i=0
    list1 = []
    while(i<data_len):
...
def File_Cal_crc32(filename):
     
    List = Bytes_To_List(Read_File(filename))
    return Cal_Crc32(List,len(List),0xc3c33c3c)
